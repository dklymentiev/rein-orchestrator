...
# Dog CLI - Unified interface for Dog workflow management
...
show_help() {
    cat << EOF
${CYAN}${BOLD}Dog CLI - Workflow Management${NC}

${BOLD}Usage:${NC}
  dog status                    # Show last 2 workflows (quick check)
  dog history [N] [RUN_ID]      # Show last N runs or details for RUN_ID
  dog logs [RUN_ID]             # Show logs for a run (latest if not specified)
  dog run <yaml>                # Execute workflow
  dog list                      # List all runs with summary
  dog clean [N]                 # Keep only last N runs (default: 10)
  dog help                      # Show this help

${BOLD}Examples:${NC}
  dog status                    # Quick status: latest + previous
  dog history                   # Show last 10 runs
  dog history 5                 # Show last 5 runs
  dog logs                      # Show logs for latest run
  dog logs 20251231-125718      # Show logs for specific run
  dog run my-workflow.yaml      # Execute workflow
  dog list                      # List all runs (summarized)
  dog clean 20                  # Keep only last 20 runs
...
cmd_history() {
    ...
    if [ ! -z "$detail" ]; then
        "$SCRIPT_DIR/dog-history.sh" "$limit" "$detail"
    else
        "$SCRIPT_DIR/dog-history.sh" "$limit"
    fi
}
...
cmd_logs() {
    ...
    if [ -f "$run_dir/dog.log" ]; then
        echo -e "${BLUE}Main log:${NC}"
        cat "$run_dir/dog.log" | tail -30
    fi
...
cmd_run() {
    ...
    if [ -z "$yaml" ]; then
        echo -e "${RED}Usage: dog run <workflow.yaml>${NC}"
        exit 1
    fi
...
cmd_list() {
    echo -e "${CYAN}${BOLD}All Dog Runs${NC}"
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
