...
show_history() {
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}Dog Workflow History - Last $LIMIT Executions${NC}"
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
    echo ""
...
show_details() {
    ...
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}Dog Workflow Details - Run $run_id${NC}"
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    if [ -f "$run_dir/dog.db" ]; then
        echo -e "${BLUE}Process Results:${NC}"
        sqlite3 -header -column "$run_dir/dog.db" \
...
if [ ! -z "$SHOW_DETAIL" ]; then
    ...
    echo "  ./dog-history.sh 10 latest     # Show latest run details"
    echo "  ./dog-history.sh 10 20251231-125718  # Show specific run details"
    echo ""
fi
