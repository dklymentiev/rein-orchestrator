...
# Rein Workflows - List and monitor active workflows
# Usage:
#   ./rein-workflows.sh              - List all active workflows (numbered)
#   ./rein-workflows.sh 3            - Show details for workflow #3
#   ./rein-workflows.sh 20251230-140000 - Show details for specific GUID
...
if [ -z "$SOCKETS" ]; then
    echo "No active Rein workflows found"
    exit 0
fi
...
if [ $# -gt 0 ]; then
    ...
    if [[ "$ARG" =~ ^[0-9]+$ ]]; then
        ...
        if [ -z "$GUID" ]; then
            echo "Workflow #$INDEX not found. Use: ./rein-workflows.sh"
            exit 1
        fi
    else
        # Treat as GUID
    ...
    # Find run directory
    RUN_DIR=$(ls -d /tmp/rein-runs/run-${GUID} 2>/dev/null | head -1)
...
for SOCKET in $SOCKETS; do
    ...
    printf "      Start: %s\n" "$START_TIME"
    printf "      Dir: %s\n" "$RUN_DIR"
    printf "      Cmd: ./rein-workflows.sh %d  (or ./rein-workflows.sh %s)\n" "$INDEX" "$GUID"
    echo ""
...
echo ""
echo "To see details: ./rein-workflows.sh <N>  (where N is number from list)"
echo "            or: ./rein-workflows.sh <GUID>"
echo "To send command: ./rein-cmd.sh <GUID> <command>"
