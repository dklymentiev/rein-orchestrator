...
from rein.agent_config import load_agent_config, validate_forbidden_behavior
from rein.log import get_console, get_logger, scrub_secrets
...
try:
    from models.validator import ValidationEngine  # noqa: F401
...
class ProcessManager:
    ...
    def load_config(self, config: dict, workflow_file: str = None):
        ...
        self._write_rein_log(
            f"CONFIG LOADED | {len(self.all_blocks)} blocks | timeout={self.timeout}s | workflow_dir={self.workflow_dir}"
        )
    ...
    def assemble_prompt(self, block: dict, team_tone: str) -> str:
        ...
        try:
            ...
            prompt = prompt_assembler.substitute_task_inputs(prompt, self.task_input, self._write_rein_log)
    ...
    def _re_evaluate_pending(self, pending: dict, spawned: set = None):
        ...
        to_add = state_machine.find_blocks_needing_repending(self.all_blocks, pending, self.completed, running)
        for block in to_add:
    ...
    def spawn_process(self, block: dict, team_tone: str = "", from_next_queue: bool = False) -> Optional[Process]:
        ...
        try:
            ...
            threading.Thread(target=self._execute_block, args=(uid, process, block, team_tone), daemon=True).start()
    ...
    def _execute_block(self, uid: str, process: Process, block: dict, team_tone: str):
        ...
        run_log.write(
            "BLOCK START", f"run={process.run_count} agent={process.agent or 'default'} phase={process.phase}"
        )
        ...
        try:
            ...
            if logic_config.get("pre"):
                ...
                if not self._run_logic(
                    logic_config["pre"],
                    save_file,
                    workflow_dir,
                    input_dir,
                    block,
                    linux_user=agent_linux_user,
            ...
            if custom_script:
                ...
                self._write_rein_log(
                    f"DEBUG | block={name} | has_agents={has_agents_or_specialist} | has_prompt={has_prompt} | team_tone={bool(team_tone)}"
                )
                if has_agents_or_specialist and has_prompt and team_tone:
                    ...
                    proc = subprocess.Popen(
                        shlex.split(command), stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env
                    )
            ...
            if block.get("routing"):
                routing_went_backward = self._apply_routing(block, name, save_file, run_log) or routing_went_backward

            # STATE MACHINE: Evaluate and trigger next block (Phase 2.5.4)
            if block.get("next") and not block.get("routing"):
                routing_went_backward = (
                    self._apply_next_state_machine(block, name, save_file, run_log) or routing_went_backward
                )
    ...
    def _apply_routing(self, block: dict, name: str, save_file: str, run_log) -> bool:
        ...
        try:
            ...
            max_runs_val = (
                next_block_config.get("max_runs", self.default_max_runs) if next_block_config else self.default_max_runs
            )

            if current_runs >= max_runs_val:
                self._write_rein_log(
                    f"ROUTING BLOCKED | {next_block_name} | run_count={current_runs} >= max_runs={max_runs_val}"
                )
                with self.lock:
            ...
            self._write_rein_log(
                f"ROUTING | {name} -> {next_block_name} | signal={matched_signal} | run={self.run_counts[next_block_name]}/{max_runs_val}"
            )
    ...
    def _apply_next_state_machine(self, block: dict, name: str, save_file: str, run_log) -> bool:
        ...
        try:
            ...
            if current_runs >= max_runs:
                self._write_rein_log(
                    f"NEXT BLOCKED | {next_block_name} | run_count={current_runs} >= max_runs={max_runs}"
                )
                with self.lock:
                    self.completed.add(next_block_name)
                    self._write_rein_log(
                        f"NEXT FORCED COMPLETE | {next_block_name} | max_runs exhausted, marking completed"
                    )
                return False
            ...
            self._write_rein_log(
                f"NEXT QUEUED | {name} -> {next_block_name} | run={self.run_counts[next_block_name]}/{max_runs}"
            )
    ...
    def _monitor_process(self, uid: str, proc: subprocess.Popen, process: Process):
        ...
        try:
            ...
            self._write_rein_log(
                f"PROCESS {'COMPLETED' if process.status == 'done' else 'FAILED'} | {name}[{uid}] | pid={process.pid} | exit_code={exit_code} | elapsed={elapsed:.1f}s | verification={verification_msg}"
            )
    ...
    def _run_step_inner(self, max_steps: int, agent_id: str = None) -> bool:
        ...
        if all_done:
            ...
            self._write_rein_log(f"STEP YIELDING | steps_used={steps_used} | remaining={remaining}")
    ...
    def _has_running_processes(self) -> bool:
        return state_machine.has_running_or_transitioning(self.processes, self.completed, self.lock)

    def _sync_completed_from_processes(self):
        state_machine.sync_completed_from_processes(self.processes, self.completed, self.lock)
    ...
    def _finalize_run(self):
        ...
        try:
            ...
            output_dir = (
                self.all_blocks[0].get("output_dir") if hasattr(self, "all_blocks") and self.all_blocks else None
            )
            if not output_dir:
            ...
            if hasattr(self, "config") and "status_path" in self.config:
                ...
                if "task_config" in self.config:
                    ...
                    if task_config.get("callback", {}).get("save_to_memory"):
                        _save_task_to_memory(output_dir, task_config.get("callback", {}).get("memory_config", {}))
    ...
    def pause_single(self, identifier: str) -> bool:
        return process_control.pause_single(
            self.processes, identifier, self.lock, self.state.save_process, self._write_rein_log
        )
    ...
    def handle_stdin_command(self, cmd: str):
        ...
        if action == "pause" and len(parts) > 1:
            ...
            if self.pause_workflow():
                self._write_rein_log("COMMAND | pause-workflow | SUCCESS")
            else:
                self._write_rein_log("COMMAND | pause-workflow | FAILED (already paused)")

        elif action == "resume-workflow":
            if self.resume_workflow():
                self._write_rein_log("COMMAND | resume-workflow | SUCCESS")
            else:
                self._write_rein_log("COMMAND | resume-workflow | FAILED (not paused)")
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    self._write_rein_log("SOCKET SERVER | stopped")
