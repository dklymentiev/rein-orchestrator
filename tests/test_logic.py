...
"""
    ...
    def test_run_python_script(self, temp_dirs, sample_script):
        ...
        runner = LogicRunner(
            task_dir=task_dir, workflow_dir=workflow_dir, task_id="test-002", logger=lambda x: logs.append(x)
        )

        output_file = os.path.join(task_dir, "output.json")

        result = runner.run(
            script_path=rel_path, output_file=output_file, block_name="test-block", block_config={"name": "test-block"}
        )
    ...
    def test_run_script_not_found(self, temp_dirs):
        ...
        runner = LogicRunner(task_dir=task_dir, workflow_dir=workflow_dir, logger=lambda x: logs.append(x))

        result = runner.run(script_path="nonexistent.py", output_file="/tmp/out.json")
    ...
    def test_run_failing_script(self, temp_dirs, failing_script):
        ...
        result = runner.run(script_path=rel_path, output_file="/tmp/out.json")
    ...
    def test_run_unknown_script_type(self, temp_dirs):
        ...
        result = runner.run(script_path="script.xyz", output_file="/tmp/out.json")
...
"""
        ...
        runner = LogicRunner(
            task_dir=task_dir, workflow_dir=workflow_dir, task_id="ctx-test", task_input={"topic": "test topic"}
        )
...
""",
        ...
        result = runner.run(script_path="slow.py", output_file=output, timeout_override=1)
...
""",
        ...
        runner = LogicRunner(task_dir=task_dir, workflow_dir=workflow_dir)
        output = os.path.join(task_dir, "out.json")
        result = runner.run(script_path="fast.py", output_file=output, timeout_override=10)
        assert result is True
...
""",
    ...
    def test_shell_script_execution(self, temp_dirs):
        ...
        with open(script_path, "w") as f:
            ...
            f.write("read ctx\n")
            f.write(
                'output=$(echo "$ctx" | python3 -c "import sys,json; print(json.load(sys.stdin)[\\"output_file\\"])")\n'
            )
...
""")
        ...
        output = os.path.join(task_dir, "out.json")
        runner.run(
            script_path="cfg.py", output_file=output, block_config={"name": "my_block", "phase": 5, "prompt": "test"}
        )
    ...
    def test_root_linux_user_forbidden(self, temp_dirs):
        ...
        logs = []
        runner = LogicRunner(task_dir=task_dir, workflow_dir=workflow_dir, logger=lambda x: logs.append(x))
        result = runner.run(script_path="ok.py", output_file=os.path.join(task_dir, "o.json"), linux_user="root")
        assert result is False
