...
# Dog command helper - Send commands to Dog via Unix domain socket
#
# Usage (auto-detect single workflow):
#   ./dog-cmd.sh pause processor-1
#   ./dog-cmd.sh resume processor-1
#   ./dog-cmd.sh status
#   ./dog-cmd.sh list
#
# Usage (explicit GUID or number for multiple workflows):
#   ./dog-cmd.sh 20251230-140000 pause processor-1
#   ./dog-cmd.sh 20251230-140500 status
#   ./dog-cmd.sh 1 list              (by number from ./dog-workflows.sh)
#   ./dog-cmd.sh 2 pause task-1
...
if [ $IS_COMMAND -eq 1 ]; then
    ...
    SOCKETS=$(ls /tmp/dog-*.sock 2>/dev/null | wc -l)

    if [ "$SOCKETS" -eq 1 ]; then
        # Exactly one socket - use it
        SOCKET=$(ls /tmp/dog-*.sock)
        ...
        # No sockets found
        echo "Error: No Dog workflows found. Start one with: python3 dog.py workflow.yaml"
        exit 1
    fi
else
    ...
    if [[ "$ARG" =~ ^[0-9]+$ ]]; then
        # Get workflow by index from dog-workflows.sh
        INDEX=$ARG
    ...
    shift
    COMMAND="$@"
    SOCKET="/tmp/dog-${GUID}.sock"
fi

if [ ! -S "$SOCKET" ]; then
    echo "Error: Dog socket not found at $SOCKET"
    echo "Is Dog running with GUID $GUID?"
    exit 1
fi
